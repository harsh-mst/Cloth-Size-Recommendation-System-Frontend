...
function cmToIn(val: string) {
  return val.replace(/(\d+)/g, (n) => Math.round(+n * 0.393701).toString()); // 1 cm = 0.393701 in [web:33][web:43]
}
...
const Index = () => {
  ...
  // height: cm in metric, feet+inches in imperial
  const [height, setHeight] = useState(""); // cm
  const [heightFeet, setHeightFeet] = useState(""); // ft (imperial)
  const [heightInches, setHeightInches] = useState(""); // in (imperial)

  const [weight, setWeight] = useState("");
  const [chest, setChest] = useState<SizeOption | "">("");
  const [waist, setWaist] = useState<SizeOption | "">("");
  const [chestCm, setChestCm] = useState(""); // numeric cm for /generate-mesh
  const [waistCm, setWaistCm] = useState(""); // numeric cm for /generate-mesh
  ...
  // Revoke previous blob URL when a new one is set to avoid memory leaks
  useEffect(() => {
    return () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta
        .env.VITE_BACKEND_URL}/predict`, {
        method: "POST",
      ...
      // Trigger 3D Model Generation via MakeHuman
      ...
      // Derive numeric chest/waist cm values:
      // Use user-entered numeric values if provided, otherwise mid-range defaults per size label
      ...
      if (modelResponse.ok) {
        // Backend returns the GLB file directly as binary
